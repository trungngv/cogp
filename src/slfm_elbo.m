function [elbo,dbeta,dw,dloghyp,dz,dm,dS] = slfm_elbo(x,y,par,cf)
%SLFM_ELBO [elbo,dbeta,dw,dloghyp,dz,dm,dS] = slfm_elbo(x,y,par,cf)
%   
...
for i=1:P
  ...
  lkl = 0.5*(logdetChol(Lmm) - logdet(par.task{i}.S) + ...
    traceABsym(Kmminv,par.task{i}.S) + par.task{i}.m'*Kmminv*par.task{i}.m); % kl-divergence part
